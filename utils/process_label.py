...
def generate_files(dict_instances):
    ...
    for key, value in dict_instances.items():
        ...
        for idx, region in enumerate(value['regions']):
            ...
            #print(f'{idx}: {points}')
        #print(len(value['regions']))
        #for shapes in value['regions']:
        #print(dict_instance)
        name = key.split('.')[0] 
...
def split_dataset(train_portion, json_files_list, images_src, images_dst):
    ...
    for train_file in train_data:
        ...
        try:
            err0 = shutil.copy(image_src_path, image_dst_path_train)
        except:
            print(f'Train 2File: {image_src_path} cannot be moved int {image_dst_path_train}. Err0: {err0}')
        #Copy label into general folder and also train folder
        try:
            err1 = shutil.copy(label_path, label_dst_path)
            err2 = shutil.copy(label_path, label_dst_path_train)
        #shutil.copy(name + '.json', '/content/drive/MyDrive/Banano/Dataset/etiquetado_instancias/02_40metros')
        except:
            print(f'File: {label_path} cannot be moved. Err1: {err1} Err2: {err2}')
