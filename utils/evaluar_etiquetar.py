...
weights_path = '/exterior/conteo_plantas/3_alturas/train/model_final.pth' 
#images_path = '/exterior/etiquetado_instancias/global/train_aumentado_2/aumentadoA_2/train/'
images_path = '/exterior/conteo_plantas/3_alturas/train/ortomosaico/'
...
class Predictor:
    ...
    def make_pred(self, im, name):
        ...
        for bbox, score in zip(bbox_raw, scores_raw):
            ...
            im = cv2.putText(im,"{:.2f}".format(score),score_height, font, 0.7,(0,0,255),2,cv2.LINE_AA)
            #cv2_imshow(im)
